...
`))
	...
	Describe("default", func() {
		...
		It("should start the server", func() {
			session, err := act()
			Expect(err).ToNot(HaveOccurred())
			Eventually(session.Err).Should(Say(`Starting quarks-statefulset \d+\.\d+\.\d+, monitoring namespaces labeled with`))
		...
		Context("when specifying monitored id for namespaces to monitor", func() {
			Context("via environment variables", func() {
				...
				It("should start for that id", func() {
					session, err := act()
					Expect(err).ToNot(HaveOccurred())
					Eventually(session.Err).Should(Say(`Starting quarks-statefulset \d+\.\d+\.\d+, monitoring namespaces labeled with 'env-test'`))
				})
			})
			...
			Context("via using switches", func() {
				It("should start for namespace", func() {
					...
					Expect(err).ToNot(HaveOccurred())
					Eventually(session.Err).Should(Say(`Starting quarks-statefulset \d+\.\d+\.\d+, monitoring namespaces labeled with 'switch-test'`))
				})
			})
		})
