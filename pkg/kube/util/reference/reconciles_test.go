	...
	Context("when getting reconciles for Ests", func() {
		...
		Context("when UpdateOnConfigChange is true", func() {
			...
			It("triggers a reconcile when a ConfigRef changes", func() {
				requests, err := reference.GetReconciles(context.Background(), client, &configMap1, false)
				Expect(err).ToNot(HaveOccurred())
				Expect(len(requests)).To(Equal(1))
			})

			It("triggers a reconcile when a ConfigKeyRef changes", func() {
				requests, err := reference.GetReconciles(context.Background(), client, &configMap2, false)
				Expect(err).ToNot(HaveOccurred())
				Expect(len(requests)).To(Equal(1))
			})

			It("triggers a reconcile when a SecretRef changes", func() {
				requests, err := reference.GetReconciles(context.Background(), client, &secret1, false)
				Expect(err).ToNot(HaveOccurred())
				Expect(len(requests)).To(Equal(1))
			})

			It("triggers a reconcile when a SecretKeyRef changes", func() {
				requests, err := reference.GetReconciles(context.Background(), client, &secret2, false)
				Expect(err).ToNot(HaveOccurred())
				Expect(len(requests)).To(Equal(1))
			})
		})
